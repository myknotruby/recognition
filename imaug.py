...
def imaug(images):
    ...
    # _per channel_.
    #ia.seed(np.random.randint(1,time.time()))
    ...
    seq = iaa.Sequential(
        [
            ...
            iaa.SomeOf((0, 5),
                [
                    sometimes(iaa.Superpixels(p_replace=(0, 0.5), n_segments=(60, 120))), # convert images into their superpixel representation
                    iaa.OneOf([
...
if '__main__' == __name__:
    ...
    #images = []
    images = np.random.randint(0, 1, (16, 112, 112, 3), dtype=np.uint8)
    for i in range(16):
        #images.append(image)
        #images[i][:] = image
        images[i][:] = cv2.imread(os.path.join(dirpath, imglist[i]))

    images_aug = imaug(images)
    #import pdb; pdb.set_trace()
    #images_aug2 = np.transpose(images_aug ,(0,3,1,2))
    for i in range(16):
        cv2.imwrite("{}.jpg".format(i),images_aug[i])
