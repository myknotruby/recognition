...
import fspherenet
import fresattnet
#import fshufflenetv2
import fmobilenetv3
import fefficientnet
#import fresidual_attention_network
import fattense152
...
def parse_args():
  ...
  parser.add_argument('--pretrained', default='', help='pretrained model to load from model,idx')
  parser.add_argument('--finetune', default='', help='finetune from model,idx')
  parser.add_argument('--exclude-layer-prefix', default='',  help='finetune from model,idx , fix layers without this prefix')
...
def get_symbol_edul_embedding(): #EDUL
  fc1, fc2, embedding = eval(config.net_name).get_symbol() #output of network, fc1=feature,fc2=var, embedding=fc1+fc2
  ...
  k=0.01
  #fc2 = fc2*fc2
  divergence = -0.5*(1+mx.symbol.log(fc2)-fc1*fc1-fc2)
  #loss_kl = mx.symbol.mean(divergence)
  loss_kl = mx.symbol.sum(divergence, axis=1).mean()
  loss_kl = k*loss_kl
  loss_kl = mx.sym.make_loss(data=loss_kl)

  out_list = [embedding, all_label, loss_kl]
...
def get_symbol_embedding():
  if config.net_output=="EDUL":
    out = get_symbol_edul_embedding()
  else:
    embedding = eval(config.net_name).get_symbol()
    #print network
    if False:
      arg_shape, out_shape, aux_shape = embedding.infer_shape(data=(1,3,112,112))
      print(out_shape)
      print(dict(zip(embedding.list_arguments(), arg_shape)))
    all_label = mx.symbol.Variable('softmax_label')
    all_label = mx.symbol.BlockGrad(all_label)
...
def get_symbol_arcface(args):
  ...
  #if dtype =='float16':
  #  embedding = mx.sym.Cast(data=embedding, dtype=np.float32)
  all_label = mx.symbol.Variable('softmax_label')
  gt_label = all_label
  is_softmax = True
...
def get_finetune_model(symbol, arg_params, aux_params, exclude_layer_prefix):
    ...
    for x in exclude_layer_prefix.split(','):
        ...
        temp = [k for k in _fixed_params_names if x not in k]
        _fixed_params_names = temp
    #import pdb; pdb.set_trace()
    return (symbol, _arg_params, _aux_params, _fixed_params_names)
...
def train_net(args):
    ...
    prefix2 = os.path.join(args.models_root, '%s-%s-%s'%(args.network, args.loss, args.dataset), 'model2')
    ...
    if len(args.finetune)>0:
      ...
      asym, arg_params, aux_params, fixed_param_names = get_finetune_model(asym, arg_params, aux_params, exclude_layer_prefix=args.exclude_layer_prefix)
    elif len(args.pretrained)>0:
      vec = args.pretrained.split(',')
      print('pretrained loading', vec)
      _, arg_params, aux_params = mx.model.load_checkpoint(vec[0], int(vec[1]))
      asym = get_symbol_arcface
      #_,_,_,fixed_param_names = get_finetune_model(asym, arg_params, aux_params, exclude_layer_prefix=args.exclude_layer_prefix)
    else:
      asym = get_symbol_arcface
    ...
    if config.count_flops:
      ...
      ## plot network graph
      #mx.viz.print_summary(esym, shape={'data':(1,3,112,112)})
      #mx.viz.plot_network(esym, shape={'data':(1,3,112,112)}, node_attrs={'shape':'oval','fixedsize':'fasl==false'}).view()
    ...
    if config.net_name=='fresnet' or config.net_name=='fmobilefacenet' or config.net_name=='fresattnet' or config.net_name=='fresnest' :
    ...
    #opt = optimizer.Nadam(learning_rate=base_lr, rescale_grad=_rescale)
    ...
    for name in config.val_targets:
      ...
      if os.path.exists(path):
        if name == 'ecuador_tp':
            data_set = testimgs_verify.load_bin(path, image_size)
        else:
    ...
    def ver_test(nbatch):
      ...
      for i in xrange(len(ver_list)):
        acc1, xnorm = 0.0, 0.0
        if ver_name_list[i]=='ecuador_tp':
            #xnorm, acc1 = testimgs_verify.test(ver_list[i], model, args.batch_size, data_extra, label_shape)
            xnorm, acc1 = testimgs_verify.test(ver_list[i], model, args.batch_size)
            print('[%s][%d]XNorm: %f' % (ver_name_list[i], nbatch, xnorm))
            print('[%s][%d]Accuracy: %1.5f' % (ver_name_list[i], nbatch, acc1))
            results.append(acc1)
        else:
    ...
    global_step = [0]
    save_step = [0]
    if len(args.pretrained)>0 and False:
      vec = args.pretrained.split(',')
      save_step[0] = int(vec[1])
      global_step[0] = save_step[0] * args.verbose

    lr_steps = [int(x) for x in args.lr_steps.split(',')]
    #lr_steps = {20000:0.01,80000:0.001,160000:0.0001,200000:0.00001}  ##lh fixed
    #lr_steps = {300000:0.001,500000:0.0001,600000:0.00001}  ##lh fixed
    print('lr_steps', lr_steps)
    def _batch_callback(param):
      #global global_step
      global_step[0]+=1
      mbatch = global_step[0]
      #if mbatch in lr_steps.keys():
      #  opt.lr = lr_steps[mbatch]
      ...
      if mbatch>=0 and mbatch%args.verbose==0:
        ...
        if do_save:
          ...
          mx.model.save_checkpoint(prefix2, 0, all_layers, arg, aux)
