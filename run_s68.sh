...
#liaohuan edit
#CUDA_VISIBLE_DEVICES='0,1,2,3' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --finetune "$PREFIX,80"  --per-batch-size 256 --exclude-layer-prefix "fc7" --lr=0.01 --lr-steps='30000,60000,100000' >"$LOGFILE-$DATASET-01" 2>&1 &
CUDA_VISIBLE_DEVICES='4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --pretrained "$PREFIX,14"  --per-batch-size 256 --exclude-layer-prefix "fc" --lr=0.01 --lr-steps='160000,320000,480000' >"$LOGFILE-$DATASET-03" 2>&1 &
#CUDA_VISIBLE_DEVICES='4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 256 --exclude-layer-prefix "fc" --lr=0.1 --lr-steps='160000,320000,480000' >"$LOGFILE-$DATASET-01" 2>&1 &
...
#CUDA_VISIBLE_DEVICES='2,3' python -u train_parall.py --dataset=$DATASET --network "$NETWORK" --pretrained "$PREFIX,1" --per-batch-size 256 --exclude-layer-prefix "fc" --lr=0.1 --lr-steps='160000,320000,480000' >"$LOGFILE-$DATASET-02" 2>&1 &
