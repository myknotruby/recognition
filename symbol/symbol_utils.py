...
def get_fc1(last_conv, num_classes, fc_type, input_channel=512):
  ...
  if fc_type=='Z':
    ...
    fc2 = mx.sym.exp(fc2)
    fc2 = mx.sym.L2Normalization(fc2)
    #fc2 = mx.symbol.Activation(data=fc2, act_type='sigmoid', name='fc2_sigmoid')
    noise = mx.symbol.random.normal_like(data=fc2, loc=0, scale=1, name='noise')
    fc2_noise = mx.sym.sqrt(fc2) * noise
    #fc2_noise = mx.ndarray.multiply(fc2,noise)
    fc1_add = fc1 + fc2_noise
    return fc1, fc2, fc1_add
  elif fc_type=='EF': #lh add 20191016
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
    #body = mx.symbol.Dropout(data=body, p=0.4)
    fc1 = mx.sym.FullyConnected(data=body, num_hidden=num_classes, name='pre_fc1')
    fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1')
    ...
    conv_6_dw = Linear(last_conv, num_filter=input_channel, num_group=input_channel, kernel=(4,4), pad=(0, 0), stride=(1, 1), name="conv_6dw4_4")  
    #conv_6_dw = mx.sym.Dropout(data=conv_6_dw, p=0.4)
