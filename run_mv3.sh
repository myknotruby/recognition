...
#CUDA_VISIBLE_DEVICES='4,5' python -u train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 120 --lr-steps='100000,200000,300000,350000' --exclude-layer-prefix 'fc' --lr 0.1 >"$LOGFILE-$DATASET-0" 2>&1 &
...
CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 128 --lr-steps='200000,320000,350000' --pretrained "$PREFIX,68" --exclude-layer-prefix 'fc' --lr 0.001 >"$LOGFILE-$DATASET-4" 2>&1 &
...
#CUDA_VISIBLE_DEVICES='7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"   --per-batch-size 1 --exclude-layer-prefix "fc7" --lr=0.1 --lr-steps='30000,60000,100000'
...
#CUDA_VISIBLE_DEVICES='2,3,4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --pretrained "$PREFIX,106"  --per-batch-size 64 --exclude-layer-prefix "fc" --lr=0.01 --lr-steps='160000,360000,480000' >"$LOGFILE-$DATASET-04" 2>&1 &
