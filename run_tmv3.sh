...
#CUDA_VISIBLE_DEVICES='0,1,2,3' python -u train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 112 --lr-steps='100000,150000' --pretrained "$PREFIX,90"  --lr 0.01 >"$LOGFILE-$DATASET-t1" 2>&1 &
CUDA_VISIBLE_DEVICES='1,2,3' python -u _train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 112 --pretrained "$PREFIX,90"  --lr 0.1 >"$LOGFILE-$DATASET-112_0.2" 2>&1 &
#CUDA_VISIBLE_DEVICES='4,5,6' python -u _train_parall.py --dataset=$DATASET --network "$NETWORK" --per-batch-size 112 --pretrained "$PREFIX,90"  --lr 0.1 >"$LOGFILE-$DATASET-112_0.5" 2>&1 &
