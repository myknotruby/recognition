...
#CUDA_VISIBLE_DEVICES='4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --pretrained "$PREFIX,69"  --per-batch-size 64 --exclude-layer-prefix "fc" --lr=0.001 --lr-steps='200000,600000,800000' >"$LOGFILE-$DATASET-000" 2>&1 &
CUDA_VISIBLE_DEVICES='1,2,3,4,5,6,7' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --pretrained "$PREFIX,41"  --per-batch-size 64 --exclude-layer-prefix "fc" --lr=0.001  >"$LOGFILE-$DATASET-003" 2>&1 &
