...
CUDA_VISIBLE_DEVICES='1,2,3' python -u train_parall.py --dataset=$DATASET --network "$NETWORK"  --pretrained "$PREFIX,34"  --per-batch-size 64 --exclude-layer-prefix "fc"  --lr=0.01 >"$LOGFILE-$DATASET-01" 2>&1 &
